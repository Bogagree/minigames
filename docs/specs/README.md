# Specs

Живые спеки. Обновляются в том же PR, что меняет поведение.

| Спека                                    | Область                             |
| ---------------------------------------- | ----------------------------------- |
| [overview.md](./overview.md)             | Продукт, домен, страницы/диалоги    |
| [architecture.md](./architecture.md)     | Стек, слои, границы модулей         |
| [story-1.md](./story-1.md)               | Скоуп и критерии Story 1            |
| [story-2.md](./story-2.md)               | Скоуп и критерии Story 2            |
| [header.md](./header.md)                 | Header guest (RSS-QS-1-4-1)         |
| [burger-menu.md](./burger-menu.md)       | Burger menu (RSS-QS-1-4-2)          |
| [hero.md](./hero.md)                     | Hero section (RSS-QS-1-4-3)         |
| [carousel.md](./carousel.md)             | Carousel / New Games (RSS-QS-1-4-4) |
| [leaderboard.md](./leaderboard.md)       | Leaderboard (RSS-QS-1-4-5)          |
| [game-developer.md](./game-developer.md) | Game developer CTA (RSS-QS-1-4-6)   |
| [footer.md](./footer.md)                 | Footer (RSS-QS-1-4-7)               |
| [auth-dialog.md](./auth-dialog.md)       | Auth dialog (RSS-QS-1-5-1 … 1-5-5)  |

Stories 3–4: добавить отдельные спеки, когда курс откроет детали (не заранее выдумывать UI). Фича-спеки Story 2 (library, game details, slider) появляются в PR соответствующего `feat/…`.
